...
var easyRent = {
    Stats: {
        changeYear: function () {
            window.location.href = "/admin/stats/index/year/" + $('#stats-year').val();
        }
    }
}
...
$(function () {
    ...
    oTable = $('.datatable table').dataTable({
        ...
        "oLanguage": {
            "sInfo": "<label class='label label-default'>Total : _TOTAL_</label>",
            "sSearch": "<span>Rechercher :</span> _INPUT_",
            "sLengthMenu": "<span>Afficher par :</span> _MENU_",
            "oPaginate": {"sFirst": "Premiere", "sLast": "Derniere", "sNext": ">", "sPrevious": "<"}
        }
    });
    ...
    oTable = $('.datatable-tools table').dataTable({
        ...
        "sDom": '<"datatable-header"Tfl><"datatable-scroll"t><"datatable-footer"ip>',
        "oLanguage": {
            "sSearch": "<span>Rechercher par :</span> _INPUT_",
            "sLengthMenu": "<span>Afficher :</span> _MENU_",
            "oPaginate": {"sFirst": "Premiere", "sLast": "Derniere", "sNext": ">", "sPrevious": "<"}
        },
        "oTableTools": {
    ...
    oTable = $('.datatable-tasks table').dataTable({
        ...
        "sDom": '<"datatable-header"fl><"datatable-scroll"t><"datatable-footer"ip>',
        "oLanguage": {
            "sSearch": "<span>Rechercher par mot</span> _INPUT_",
            "sLengthMenu": "<span>Afficher par:</span> _MENU_",
            "oPaginate": {"sFirst": "Premiere", "sLast": "Derniere", "sNext": ">", "sPrevious": "<"}
        },
        "aoColumnDefs": [
    ...
    var oTable = $('.datatable-add-row table').dataTable({
        ...
        "sDom": '<"datatable-header"fl><"datatable-scroll"t><"datatable-footer"ip>',
        "oLanguage": {
            "sSearch": "<span>Filtrer tous:</span> _INPUT_",
            "sLengthMenu": "<span>Afficher :</span> _MENU_",
            "oPaginate": {"sFirst": "Premiere", "sLast": "Derniere", "sNext": ">", "sPrevious": "<"}
        }
    });
    ...
    $('.dataTables_filter input[type=text]').attr('placeholder', 'Chercher ...');
    ...
    $(".select-full").select2();
    $(".select-sim-not-used").select2({
        ajax: {
            url: "/app_dev.php/sim/json/all",
            dataType: 'json',
            delay: 250,
            data: function (params) {
                return {
                    query: params.term, // search term
                    dateBegin: $('#abonnement_dateDebut_year').val() + '-' + $('#abonnement_dateDebut_month').val() + '-' + $('#abonnement_dateDebut_day').val(), // search term
                    dateEnd: $('#abonnement_dateFin_year').val() + '-' + $('#abonnement_dateFin_month').val() + '-' + $('#abonnement_dateFin_day').val(), // search term
                    page: params.page
                };
            },
    ...
    $('#reportrange').daterangepicker(
        ...
        function (start, end) {
            ...
            $("#date-range-from").val(start.format('YYYY-MM-DD'));
            $("#date-range-to").val(end.format('YYYY-MM-DD'));
...
function getModele(Stringurl, marque) {
    var myData = "marque=" + marque;
    $.ajax({
        type: "POST",
        url: Stringurl,
        data: myData,
...
function getPrix(Stringurl, modelvh) {
    var myData = "modelvh=" + modelvh;
    $.ajax({
        type: "POST",
        url: Stringurl,
        data: myData,
...
function getOptions(Stringurl, vh) {
    var myData = "vh=" + vh;
    $.ajax({
        type: "POST",
        url: Stringurl,
...
function getDispo(Stringurl, vh) {
    var myData = "vehicule=" + vh;
    $.ajax({
        type: "POST",
        url: Stringurl,
        data: myData,
...
function getResa(Stringurl, resa) {
    var myData = "idrs=" + resa;
    $.ajax({
        type: "POST",
        url: Stringurl,
        data: myData,
        success: function (msg) {
            $("#default-modal").html(msg);
        },
        error: function () {
            alert("AJAX call an epic failure");
        }
